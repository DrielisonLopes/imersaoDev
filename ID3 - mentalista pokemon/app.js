...
const colorsNamesList = ['Squirtle', 'Gestily', 'Bullbassaur', 'Psyduck', 'Diglet', 'Pikachu', 'Jigglypuff', 'Charmander', 'Abra', 'Snorlax', 'Wobbuffet', 'Machamp'];
...
function validateColor(e) {
    ...
    if (coins > 0 && !win) {
        if (elementClicked.dataset.id == randomMove) {
            ...
            win = true;
            $resultH2.innerText = `Parabéns! Era o ${colorsNamesList[randomMove]} mesmo que estava pensando!`;
        ...
        if (coins <= 0) {
            $resultH2.innerHTML = `Suas tentativas acabaram, o Pokémon correto era <br><mark style="background-color:${correctColor};color:#f8f7ff;">${colorsNamesList[randomMove]}</mark>`;
